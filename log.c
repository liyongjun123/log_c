...
void elog(uint8_t level, char *file, int line, const char *format, ...)
{
    ...
    // //行号
    // if (g_log_option & LOG_LINE)
    // {
    //     //打印
    //     printf("[line:%d]", __LINE__);
    //     //写入log文件
    //     fprintf(pFile, "[line:%d]", __LINE__);
    // }
...
static int analyze_conf(char *buf)
{
    ...
    {
        if (get_option(PRINT_D))
            printf("print_enable analyze error\n");
    }
    ...
    if (pJson)
    {
        ...
        {
            set_option(PRINT_TIME);
        }
        else
        {
            clear_option(PRINT_TIME);
            if (get_option(PRINT_D))
            {
                printf("type is error : 0x%02x ", pJson->type);
                print_cjson_type(pJson->type);
    ...
    if (pJson)
    {
        if (pJson->type == cJSON_True)
        {
            set_option(PRINT_LINE);
        }
        else
        {
            clear_option(PRINT_LINE);
            if (get_option(PRINT_D))
            {
                printf("type is error : 0x%02x ", pJson->type);
                print_cjson_type(pJson->type);
